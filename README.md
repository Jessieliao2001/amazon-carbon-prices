...
#### HMC Part
10. Run in server: (update with new `pee`)
```bash
bash bash_files/hmc_sampling.sh
bash bash_files/hmc_relative_entropy.sh
```

11. Run: (update with new `pee`, change in `scripts/conduction_hmc.py`)
```bash
...
- use `prob_hh` and `prob_ll` to update `pysrc/mpc/mpc_simulating.py` line 187 to the end for 
  - xi=0.5, 1
  - b=0,10,15,20,25
  - unconstrained and constrained

- rerun below with updated probability
```bash
bash bash_files/mpc_prepare.sh
```
- rerun (pee, xi)*2 (unconstrained, constrained) with trig=1
```bash
bash bash_files/mpc_hmc.sh
```
15. NEW
