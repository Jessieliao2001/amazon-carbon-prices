...
aux.prices <- c(6.7, 16.7, 21.7, 26.7, 31.7)
...
prediction.1043SitesModel <- purrr::map_df(
  .x = aux.prices,
  .f = ~ {
    file_path <- here::here("output/optimization/det/gurobi/1043sites/pa_41.11",
                            paste0("pe_", .x, "/Z.txt"))
...
# GENERATE MAPS
dir.create(here::here("plots/1043-det"), recursive = TRUE, showWarnings = FALSE)
...
# gamma_1043Sites
gamma_1043Sites <-
  ggplot2::ggplot(data = prediction.1043SitesModel %>% dplyr::filter(time == 0, p_e == 6.7) %>%
