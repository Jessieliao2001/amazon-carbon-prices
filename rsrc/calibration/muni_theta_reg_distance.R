...
# # Assign each municipality to the basin whose centroid is nearest
# # to the municipality's own centroid
# muni_centroids <- st_centroid(muni_data)
# basin_centroids <- st_centroid(map_basin)
# nearest_idx <- st_nearest_feature(muni_centroids, basin_centroids)
# muni_data$FEATURE_ID <- map_basin$FEATURE_ID[nearest_idx]

# Assign each municipality to the basin with the largest overlap area
# keep only basin id
map_basin_tf <- map_basin %>% select(FEATURE_ID)

# create a temporary municipality id in case there is no unique id handy
muni_data <- muni_data %>%
  mutate(.muni_id_tmp = row_number())

# optional but often helpful if geometry issues exist
muni_data    <- st_make_valid(muni_data)
map_basin_tf <- st_make_valid(map_basin_tf)

# intersection between municipality polygons and basin polygons
inter <- st_intersection(
  muni_data %>% select(.muni_id_tmp),
  map_basin_tf
)
...
# for each municipality, keep the basin with the largest overlap
muni_basin_max <- inter %>%
  st_drop_geometry() %>%
  group_by(.muni_id_tmp) %>%
  slice_max(overlap_area, n = 1, with_ties = FALSE) %>%
  ungroup() %>%
  select(.muni_id_tmp, FEATURE_ID)

# join back to muni_data
muni_data <- muni_data %>%
  left_join(muni_basin_max, by = ".muni_id_tmp") %>%
  select(-.muni_id_tmp)
...
# Municipalities whose nearest-centroid basin has no regression observations
# are reassigned to the nearest basin centroid that does appear in the
# regression sample.
...
orphan_mask <- !muni_data$FEATURE_ID %in% reg_basins
